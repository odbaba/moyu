...
import {
  ...
  upgradePetLevel,
  petTypeBaseScore
} from '../src/utils/petGenerator.js';
import { generateShopPet } from '../src/utils/shopUtils.js';
import { selectLegendaryPrize, selectHighPrize, selectMediumPrize } from '../src/utils/lotterySystem.js';
...
function testInitialRatingCalculation() {
  ...
  // 测试用例1: 初始生命值评分
  const chp1 = 120;
  const expectedChpScore1 = (chp1 - 100) * 2;
  ...
  // 测试用例2: 初始最小攻击评分
  const cxgj2 = 20;
  const expectedXgjScore2 = (cxgj2 - 15) * 2;
  ...
  // 测试用例3: 初始最大攻击评分
  const cdgj3 = 35;
  const expectedDgjScore3 = (cdgj3 - 25) * 2;
  ...
  // 测试用例4: 初始防御评分
  const cfy4 = 15;
  const expectedFyScore4 = (cfy4 - 10) * 2;
...
function testGrowthRatingCalculation() {
  ...
  // 测试用例2: 差值>10的情况
  const cz_xgj2 = 25;
  const expectedXgjScore2 = (15 - 10) * 100 + 200;
  ...
  // 测试用例3: 最大攻击成长评分
  const cz_dgj3 = 30;
  const expectedDgjScore3 = (15 - 10) * 100 + 200;
