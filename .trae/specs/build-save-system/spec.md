...
#### Scenario: 存档版本不兼容
- **WHEN** 存档版本号与当前游戏版本不匹配
- **THEN** 系统清除旧存档，"继续游戏"按钮禁用

### Requirement: 版本兼容性检查
系统 SHALL 在加载存档时检查版本号，版本不兼容时清除旧存档。

#### Scenario: 版本检查
- **WHEN** 系统检查存档版本
- **THEN** 如果存档版本与当前版本不匹配，清除存档数据

### Requirement: 开始游戏清除存档
系统 SHALL 在"开始游戏"时清除旧存档，确保全新开始。

#### Scenario: 开始新游戏
- **WHEN** 用户点击"开始游戏"
- **THEN** 清除所有存档数据，以默认初始状态进入游戏
