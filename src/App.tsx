...
import type { BattleResult, Interactable, InventoryItem, EnemyInteractable, NPCInteractable, Pet, CharacterData, EquipmentSlotType, EquipmentDetail, EquipmentItem, TimeSystem, SkillDetail, EnemyData, ActionInteractable, PlayerResources, PrincessRelationship, DailyTaskStatus, GemItem, RefineResult, BattleCharacter, BattlePet } from './types'
...
import { interactableConfig, generateBossInteractables } from './data/interactableData'
...
import { equipmentItemToDetail, equipmentDetailToItem } from './utils/equipmentConverter'
...
import { bossTemplates, bossSpawnConfigs } from './data/bossData';

// 导入战利品工具函数
import { calculateLoot, mergeLootResults } from './utils/lootUtils';

// 导入 NPC 相关工具函数
import { getDemonArmyDialogue, performChat, performGift, receiveSundayGift, receiveConfidantGift } from './utils/princessRelationUtils';
import { claimMilitaryPay, queryBattleExp, queryMilitaryIntel, getMilitaryRankDescription } from './utils/militaryRankUtils';
import { getNobleRankSystemDescription, getTradeSystemMessage, claimNobleReward } from './utils/nobleRankUtils';
...
function App() {
  ...
  // 军衔和战功状态
  const [militaryRank, setMilitaryRank] = useState(0)        // 军衔等级（0-11）
  const [battleExp, setBattleExp] = useState(0)              // 累计战功
  ...
  const [merit, setMerit] = useState(0)                      // 累计功勋
  const [lastNobleRewardClaimTime, setLastNobleRewardClaimTime] = useState<number | null>(null) // 上次领取爵位奖励的时间
  
  // 日常任务状态
  const [dailyTaskStatus, setDailyTaskStatus] = useState<DailyTaskStatus | null>(null) // 当前任务状态
