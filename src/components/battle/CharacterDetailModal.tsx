import './battle.css';

import React from 'react';

import type { BattleCharacter, Buff, BuffType } from '../../types';

/**
 * 角色详情弹窗组件属性接口
 * 定义组件接收的参数类型
 */
interface CharacterDetailModalProps {
  /**
   * 是否显示弹窗
   */
  isVisible: boolean;
  /**
   * 关闭弹窗的回调函数
   */
  onClose: () => void;
  /**
   * 要显示的角色对象
   */
  character: BattleCharacter | null;
}

/**
 * 获取增益效果对应的颜色
 * 根据增益类型返回不同的颜色类名
 * @param type 增益效果类型
 * @returns 颜色类名
 */
const getBuffColor = (type: BuffType): string => {
  switch (type) {
    case 'combat_power':
      return 'buff-combat-power';
    case 'attack':
      return 'buff-attack';
    case 'defense':
      return 'buff-defense';
    case 'speed':
      return 'buff-speed';
    case 'critical':
      return 'buff-critical';
    case 'dodge':
      return 'buff-dodge';
    default:
      return 'buff-default';
  }
};

/**
 * 获取增益效果的图标
 * 根据增益类型返回对应的图标
 * @param type 增益效果类型
 * @returns 图标字符串
 */
const getBuffIcon = (type: BuffType): string => {
  switch (type) {
    case 'combat_power':
      return '⚔️';
    case 'attack':
      return '🗡️';
    case 'defense':
      return '🛡️';
    case 'speed':
      return '💨';
    case 'critical':
      return '💥';
    case 'dodge':
      return '✨';
    default:
      return '⬆️';
  }
};

/**
 * 角色详情弹窗组件
 * 用于显示玩家角色的详细信息，包括基础属性、技能列表、增益效果等
 * 以表格形式展示各项数据
 */
const CharacterDetailModal: React.FC<CharacterDetailModalProps> = ({
  isVisible,
  onClose,
  character
}) => {
  /**
   * 处理点击遮罩层关闭弹窗
   */
  const handleOverlayClick = (e: React.MouseEvent) => {
    if (e.target === e.currentTarget) {
      onClose();
    }
  };

  /**
   * 计算生命值百分比
   */
  const getHpPercent = (): string => {
    if (!character) return '0%';

    return `${Math.round((character.currentHp / character.maxHp) * 100)}%`;
  };

  /**
   * 计算体力值百分比
   */
  const getStaminaPercent = (): string => {
    if (!character) return '0%';

    return `${Math.round((character.currentStamina / character.maxStamina) * 100)}%`;
  };

  // 如果弹窗不可见或没有角色数据，则不渲染
  if (!isVisible || !character) return null;

  return (
    <div
      className="character-detail-modal-overlay"
      onClick={handleOverlayClick}
    >
      <div className="character-detail-modal-content">
        {/* 关闭按钮 */}
        <button
          className="character-detail-close-button"
          onClick={onClose}
          aria-label="关闭"
        >
          ×
        </button>

        {/* 角色头部信息：图标和名称 */}
        <div className="character-detail-header">
          <div className="character-detail-icon-wrapper">
            <span className="character-detail-emoji">🧙</span>
          </div>
          <h3 className="character-detail-name">
            {character.name}
          </h3>
        </div>

        {/* 详细信息表格 */}
        <div className="character-detail-table">
          {/* 第一行：名字和等级 */}
          <div className="character-detail-row">
            <div className="character-detail-cell">
              <span className="cell-label">名字</span>
              <span className="cell-value">{character.name}</span>
            </div>
            <div className="character-detail-cell">
              <span className="cell-label">等级</span>
              <span className="cell-value">Lv.{character.level}</span>
            </div>
          </div>

          {/* 第二行：生命值 */}
          <div className="character-detail-row">
            <div className="character-detail-cell full-width">
              <span className="cell-label">生命值</span>
              <span className="cell-value hp-value">{Math.round(character.currentHp)}/{Math.round(character.maxHp)}</span>
              <span className="cell-percent">({getHpPercent()})</span>
            </div>
          </div>

          {/* 第三行：体力值 */}
          <div className="character-detail-row">
            <div className="character-detail-cell full-width">
              <span className="cell-label">体力值</span>
              <span className="cell-value stamina-value">{character.currentStamina}/{character.maxStamina}</span>
              <span className="cell-percent">({getStaminaPercent()})</span>
            </div>
          </div>

          {/* 第四行：攻击力 */}
          <div className="character-detail-row">
            <div className="character-detail-cell full-width">
              <span className="cell-label">攻击力</span>
              <span className="cell-value attack-value">{character.attackMin}-{character.attackMax}</span>
            </div>
          </div>

          {/* 第五行：防御力和战斗力 */}
          <div className="character-detail-row">
            <div className="character-detail-cell">
              <span className="cell-label">防御力</span>
              <span className="cell-value defense-value">{character.defense}</span>
            </div>
            <div className="character-detail-cell">
              <span className="cell-label">战斗力</span>
              <span className="cell-value combat-power-value">{character.combatPower}</span>
            </div>
          </div>

          {/* 第六行：闪避率和幸运值 */}
          <div className="character-detail-row">
            <div className="character-detail-cell">
              <span className="cell-label">闪避率</span>
              <span className="cell-value">{character.dodgeRate}%</span>
            </div>
            <div className="character-detail-cell">
              <span className="cell-label">幸运值</span>
              <span className="cell-value">{character.luck}</span>
            </div>
          </div>

          {/* 技能列表 */}
          {character.skills && character.skills.length > 0 && (
            <div className="character-detail-skills">
              <div className="skills-label">技能列表</div>
              <div className="skills-list">
                {character.skills.map((skill, index) => (
                  <div key={skill.id || index} className="skill-item">
                    <span className="skill-name">{skill.name}</span>
                    <span className="skill-info">
                      {skill.attackType === 'single' && '单体攻击'}
                      {skill.attackType === 'aoe' && '群体攻击'}
                      {skill.attackType === 'multi' && `多段攻击(${skill.hitCount}次)`}
                      {skill.attackType === 'buff' && '增益技能'}
                      {skill.staminaCost > 0 && ` | 体力: ${skill.staminaCost}`}
                    </span>
                  </div>
                ))}
              </div>
            </div>
          )}

          {/* 增益效果列表 */}
          {character.buffs && character.buffs.length > 0 && (
            <div className="character-detail-buffs">
              <div className="buffs-label">增益效果</div>
              <div className="buffs-list">
                {character.buffs.map((buff: Buff, index: number) => (
                  <div key={buff.id || index} className={`buff-item ${getBuffColor(buff.type)}`}>
                    <span className="buff-icon">{getBuffIcon(buff.type)}</span>
                    <span className="buff-name">{buff.name}</span>
                    <span className="buff-info">
                      +{buff.value}% ({buff.duration}回合)
                    </span>
                  </div>
                ))}
              </div>
            </div>
          )}
        </div>
      </div>
    </div>
  );
};

export default CharacterDetailModal;
