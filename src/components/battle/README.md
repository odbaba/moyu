...
- **职责**:
  - 管理战斗状态（玩家、敌人、回合、日志等）
  - 处理玩家行动选择（技能释放后自动选择生命值最低的敌人作为目标）
  - 支持自动战斗模式（勾选后自动执行技能）
  - 执行敌方AI行动
...
人物幸运值是角色在战斗中的重要属性，决定了幻兽受到致命伤害时的特殊机制。
...
#### 交互逻辑
- **点击角色/幻兽/敌人**：查看详情（技能自动选择最低血量敌人，无需手动选择目标）
...
3. 九宫格布局用于显示角色位置，支持点击查看详情
4. 所有动画效果使用CSS实现，性能优化
5. 移动端已优化，九宫格高度为屏幕的1/3
6. 技能冷却系统确保技能平衡使用
7. 战斗力系统提供更直观的战力对比
8. **自动瞄准机制**：释放技能后自动选择生命值最低的存活敌人作为目标，无需手动点击选择目标
9. **自动战斗机制**：行动按钮区域增加自动战斗勾选框，勾选后在玩家回合自动选择技能并执行，技能按钮隐藏，显示"自动战斗中"提示；取消勾选恢复手动操作。技能选择策略：优先使用飞天连斩/高级飞天连斩、飞天连斩存在但体力不足时使用风斩/高级风斩、否则使用伤害倍率最高的技能
