 ...
 * @returns 掉落描述字符串数组
 */
export function getWarSoulDropDescription(
  monsterId: string,
  warSoulSystemEnabled: boolean
): string[] {
  ...
  if (config) {
    for (const drop of config.drops) {
      // 如果战魂系统未开启且不是忽略系统状态的掉落，则跳过
      if (!drop.ignoreSystemEnabled && !warSoulSystemEnabled) {
        continue;
      }

      const itemName = drop.itemType === 'zhanHunZhiXin' ? '战魂之心' : '战魂晶石';
      descriptions.push(`${itemName} (${drop.dropRate}%)`);
    }
  }
  ...
  if (isBossMonster(monsterId) && warSoulSystemEnabled) {
    const hasJingShiDrop = config?.drops.some((d) => d.itemType === 'zhanHunJingShi');
    if (!hasJingShiDrop) {
      descriptions.push('战魂晶石 (50% BOSS额外掉落)');
    }
  }
