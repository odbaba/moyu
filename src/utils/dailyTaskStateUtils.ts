 ...
 * @returns 任务状态摘要文本
 */
export function getTaskStateSummary(taskState: DailyTaskState): string {
  const lines: string[] = [];
  ...
  // 收集宝石任务（周一、周二）
  lines.push(`收集宝石任务：${getTaskStateDescription(taskState, 'collect')}`);

  // 训练幻兽任务（周三、周四）
  lines.push(`训练幻兽任务：${getTaskStateDescription(taskState, 'train')}`);

  // 突袭任务（周五）
  lines.push('突袭任务：✅ 可以接受任务');

  // PK赛任务（周六）
  lines.push('PK赛任务：✅ 可以接受任务');

  // 地下城任务（周日）
  lines.push(`地下城任务：${getTaskStateDescription(taskState, 'dungeon')}`);
 ...
 * 检查地下城怪物是否存在
 * @param taskState 任务状态
 * @param floor 地下城层数（1, 2, 3）
 * @param monsterIndex 怪物索引（1, 2, 3）
 * @returns 怪物是否存在
 */
export function isDungeonMonsterExists(
  taskState: DailyTaskState,
  floor: number,
  monsterIndex: number
): boolean {
  const key = `rw_gw${floor}_${monsterIndex}` as keyof DailyTaskState;

  return taskState[key] as boolean;
}
 ...
 * @param taskState 当前任务状态
 * @param floor 地下城层数
 * @param monsterIndex 怪物索引
 * @returns 更新后的任务状态
 */
export function defeatDungeonMonster(
  taskState: DailyTaskState,
  floor: number,
  monsterIndex: number
): DailyTaskState {
 ...
 * 检查雪域边境怪物是否存在
 * @param taskState 任务状态
 * @param monsterIndex 怪物索引（1-5）
 * @returns 怪物是否存在
 */
export function isSnowfieldMonsterExists(
  taskState: DailyTaskState,
  monsterIndex: number
): boolean {
  const key = `gw_xybj_${monsterIndex}` as keyof DailyTaskState;
 ...
 * @returns 更新后的任务状态
 */
export function defeatSnowfieldMonster(
  taskState: DailyTaskState,
  monsterIndex: number
): DailyTaskState {
  const key = `gw_xybj_${monsterIndex}` as keyof DailyTaskState;
 ...
 * 增加任务进度
 * @param taskState 当前任务状态
 * @param increment 增加的进度值（默认为1）
 * @returns 更新后的任务状态
 */
export function incrementTaskProgress(
  taskState: DailyTaskState,
  increment: number = 1
): DailyTaskState {
  return {
    ...taskState,
    taskProgress: taskState.taskProgress + increment,
  };
}
 ...
 * @returns 更新后的任务状态
 */
export function clearCurrentTask(taskState: DailyTaskState): DailyTaskState {
  return {
    ...taskState,
    currentTaskId: undefined,
    taskAcceptedAt: undefined,
